...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            console.log("Actual word = " + theAnswer);
            

            // TODO 15
            // Update the corresponding wordSubmission in the model
            for (var idx = 0; idx < model.wordSubmissions.length; idx++) {
                if (model.wordSubmissions[idx].word === word) {
                    model.wordSubmissions[idx].isRealWord = theAnswer
                }
            };
...
function render() {
    ...
    if (gameOver) {
        ...
        model.currentAttempt = "";
        $("#textbox").val(model.currentAttempt).attr("disabled", true);
    }
}
